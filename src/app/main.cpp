...
void get_method_handler(const shared_ptr<Session> session) {
    ...
    return session->close(OK, json, {{"Content-Length", to_string(json.size())},
...
void post_method_handler(const shared_ptr<Session> session) {
    ...
    session->fetch(content_length, [request](const shared_ptr<Session> session, const Bytes &body) {
        ...
        for (std::vector<unsigned char>::size_type i = 0; i < body.size(); i++) {
            if (body[i] != '"') {
                if (int(body[i] > 96)){
                    if (int(body[i] < 123)){
                        tmp += body[i];
        ...
        string json = j.dump();
        session->close(OK, json, {{"Content-Length", to_string(json.size())},
                                  {"Content-Type",   "application/json"}});
    });
}
