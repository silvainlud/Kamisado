...
int main(int, const char **)
{
  ...
  std::vector<std::vector<int> > samples = {
      {1, 14, 26, 4}, {3, 15, 21, 4}, {3, 19, 33, 1}, {3, 18, 28, 2},
      {1, 11, 31, 2}, {3, 16, 43, 1}, {2, 11, 22, 4}, {3, 19, 50, 4},
      {1, 12, 34, 5}, {2, 11, 23, 2}, {2, 10, 38, 2}, {3, 16, 40, 5},
      {3, 12, 48, 3}, {1, 13, 40, 3}, {2, 11, 35, 3}, {2, 18, 35, 3},
      {3, 16, 35, 3}, {3, 10, 28, 2}, {1, 13, 20, 4}, {2, 15, 47, 4},
      {3, 17, 27, 5}, {3, 18, 41, 1}, {1, 14, 36, 5}, {1, 17, 24, 3},
      {1, 18, 42, 2}, {2, 14, 25, 5}, {1, 12, 30, 4}, {3, 12, 35, 2},
      {3, 11, 32, 2}, {2, 12, 41, 1}, {1, 13, 46, 4}, {1, 19, 45, 5},
      {2, 15, 34, 5}, {1, 13, 33, 1}, {2, 18, 49, 4}, {3, 11, 45, 1},
      {2, 11, 30, 5}, {3, 17, 47, 2}, {3, 12, 27, 5}, {2, 11, 46, 2},
      {1, 13, 23, 3}, {2, 17, 29, 4}, {3, 15, 28, 1}, {3, 16, 39, 3},
      {2, 14, 44, 1}, {3, 15, 33, 2}, {2, 10, 48, 1}, {2, 15, 39, 5},
      {1, 10, 42, 1}, {2, 19, 50, 3}, {3, 11, 50, 1}, {1, 16, 38, 3},
      {1, 14, 26, 2}, {1, 12, 24, 1}, {1, 15, 37, 3}, {1, 14, 20, 3},
      {1, 13, 29, 5}, {1, 14, 37, 4}, {3, 13, 44, 1}, {2, 12, 46, 4},
      {1, 12, 40, 1}, {2, 16, 39, 2}, {2, 19, 49, 2}, {2, 17, 22, 2},
      {3, 17, 30, 4}, {3, 16, 29, 1}, {1, 17, 44, 5}, {3, 19, 24, 4},
      {1, 16, 22, 3}, {2, 16, 45, 3}, {1, 19, 21, 4}, {2, 16, 20, 2},
      {2, 13, 22, 4}, {2, 15, 31, 5}, {1, 19, 27, 2}, {3, 19, 47, 1},
      {3, 10, 48, 4}, {2, 11, 26, 5}, {1, 13, 42, 5}, {2, 13, 37, 5},
      {3, 14, 39, 4}, {3, 17, 49, 2}, {3, 18, 43, 5}, {2, 18, 32, 1},
      {1, 18, 36, 5}, {1, 10, 32, 3}, {1, 14, 27, 3}, {3, 12, 38, 2},
      {2, 17, 24, 1}, {2, 10, 33, 5}, {3, 10, 31, 3}, {3, 10, 21, 1},
      {2, 18, 25, 3}, {1, 17, 43, 2}, {1, 18, 41, 3}, {2, 13, 34, 3}
  };

  for (const std::vector<int> &sample: samples) {
    std::string
        file_name = "result_" + std::to_string(sample[1]) + "_" + std::to_string(sample[2]) + "_" +
        std::to_string(sample[3]) + "_" + std::to_string(sample[0]);
